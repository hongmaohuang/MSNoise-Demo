...
CSV_PATH="reykjanes_stations_2020_2023_precheck.csv"
OUTPUT_DIR="Precheck_Waveforms"
FAILED_CSV="failed_precheck_waveform_downloads.csv"
TIMEOUT=60
LIMIT_TASKS=""
DATE_FROM=""
DATE_TO=""
DRY_RUN=0
declare -a STATION_PATTERNS=()
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --station-pattern) STATION_PATTERNS+=("$2"); shift 2 ;;
...
task_generator() {
  python3 - "$CSV_PATH" "$DATE_FROM" "$DATE_TO" "$LIMIT_TASKS" "${STATION_PATTERNS[@]}" <<'PY'
import csv
import fnmatch
import sys
